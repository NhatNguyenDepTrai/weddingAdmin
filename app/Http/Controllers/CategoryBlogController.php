...
class CategoryBlogController extends Controller
{
    ...
    {
        $data = CategoryBlog::with('listImages')->get();
        foreach ($data as $key => $value) {
            ...
            $value->view = ListView::where('tb', 'category_blogs')
                ->where('id_tb', $value->id)
                ->count();
        }
        $trashCount = CategoryBlog::onlyTrashed()->count();
    ...
    function showIndex()
    {
        ...
        return Inertia::render('CategoryBlog/Show', ['data' => $data, 'trashCountNumber' => $trashCount]);
    }
    function create(Request $rq)
    {
        ...
        $id_tb = CategoryBlog::create($data)->id;
        ListSlug::create([
            'tb' => $tb,
            'id_tb' => $id_tb,
            'name' => $rq->name,
    ...
    {
        $data = CategoryBlog::with('listImages')->find($id);

        return Inertia::render('CategoryBlog/Edit', ['data' => $data]);
    }
    ...
    function showTrash()
    {
        $data = CategoryBlog::with('listImages')
            ->onlyTrashed()
            ->get();
        foreach ($data as $key => $value) {
            $value->create_time = Carbon::parse($value->created_at)->format('H:i , d/m/Y ');
        ...
        return Inertia::render('CategoryBlog/Trash', ['data' => $data, 'trashCount' => $trashCount]);
    }
}
